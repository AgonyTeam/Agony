...
function pyramidHead:cacheUpdate (player,cacheFlag)
	if (player:HasCollectible(item_PyramidHead)) and (cacheFlag == CacheFlag.CACHE_SPEED) then
...
function pyramidHead:onUpdate(player)
	...
	if pyramidHead.hasItem == false and player:HasCollectible(item_PyramidHead) then
...
function pyramidHead:fearEnemies(npc)
	...
	if player:HasCollectible(item_PyramidHead) then
...
function pyramidHead:darken()
	if Game():GetPlayer(0):HasCollectible(item_PyramidHead) then
		Game():Darken(1, 1)
	end
end
