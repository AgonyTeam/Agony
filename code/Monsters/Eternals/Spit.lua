...
function eternalSpit:tearDeath(pos, vel, spawner, ent)
	...
	Agony:fireIpecacTearProj(0, Agony.TearSubTypes.ETERNAL, pos, Agony:calcTearVel(pos, Isaac.GetPlayer(0).Position, v), {Scale = 0.5, SpawnerEntity = spawner})
end
...
function eternalSpit:ai_dmg(ent, dmg, flags, src, countdown)
	if ent.Variant == 1 and ent.SubType == 15 and dmg >= ent.HitPoints then
		Agony:addDelayedFunction(Agony:getCurrTime()+3, function (data)
			...
			Agony:fireIpecacTearProj(0, Agony.TearSubTypes.ETERNAL, pos, Agony:calcTearVel(pos, Isaac.GetPlayer(0).Position, v), tearConf)
